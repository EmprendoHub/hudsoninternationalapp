...
const AffiTermsComp = ({ affiTermsDic }) => {
  return (
    <div className="flex flex-col items-start justify-start h-full mt-5 px-10 maxmd:px-5">
      ...
      {affiTermsDic.textArray.map((item, index) => (
        <div key={item.section.title + toString(index)} className=" my-3">
          <h2 className=" font-primary text-2xl mb-2">{item.section.title}</h2>

          {item?.section?.text && (
            <p className="text-sm font-secondary mb-2"> {item.section?.text}</p>
          )}
