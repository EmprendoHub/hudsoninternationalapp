...
const UpdateProductDetails = ({ product }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (title === "") {
      setError("Por favor complete el nombre del producto para continuar.");
      return;
    }
    if (brand === "") {
      setError("Por favor complete la marca del producto para continuar.");
      return;
    }
    if (gender === "") {
      setError("Por favor agrega el genero del producto para continuar.");
      return;
    }
    if (description === "") {
      setError("Por favor agregue una description para continuar.");
      return;
    }
    if (sizes === "") {
      setError("Por favor agrega los tamaños del producto para continuar.");
      return;
    }

    if (category === "") {
      setError("Por favor agrega la categoría del producto para continuar.");
      return;
    }
    if (price <= 0) {
    ...
    if (inputImageFields > 0) {
      inputImageFields.map((field) => {
        if (field.i_color === "") {
          setError("Por favor agrega nombre al bono(s) para continuar.");
          return;
        }

        if (field.i_file === "") {
          setError("Por favor agrega imágenes al bono(s) para continuar.");
          return;
        }
      });
    } else {
    ...
    try {
      ...
      try {
        ...
        setError("Error actualizando Producto. Por favor Intenta de nuevo.");
      }
    } catch (error) {
      console.log(error);
    }
          ...
          <div className="flex flex-row maxsm:flex-col-reverse items-start justify-start gap-x-5  maxmd:py-4 maxmd:px-5 maxsm:px-0">
            ...
            <div className="description-class w-1/2 maxsm:w-full h-full ">
              <div className="flex flex-col items-start justify-start pt-10 maxsm:pt-2 gap-y-10 w-[90%] maxmd:w-full pb-10">
                <h1 className="text-xl maxmd:text-3xl font-semibold font-primary text-black">
                  Actualizar Producto
                </h1>
                ...
                >
                  <p className="text-7xl maxsm:text-5xl font-semibold font-primary">
                    {product?.title}
                  </p>
