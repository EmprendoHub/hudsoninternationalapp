...
const UpdatePostDetails = ({ post }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (title === "") {
      setError("Por favor complete el nombre del publicación para continuar.");
      return;
    }

    if (content === "") {
      setError("Por favor agregue contenido para continuar.");
      return;
    }
    if (summary === "") {
      setError(
    ...
    if (category === "") {
      setError("Por favor agrega la categoría del publicación para continuar.");
      return;
    }
    ...
    if (inputImageFields > 0) {
      ...
      if (inputImageFields[0].i_file === "") {
        setError("Por favor agrega imágenes a la publicación para continuar.");
        return;
      }
    }
      ...
      try {
        ...
        if (res.ok) {
          Swal.fire({
            ...
            color: "#0D121B",
            toast: true,
            text: `La publicación se actualizo exitosamente`,
        ...
        setError("Error actualizando Publicación. Por favor Intenta de nuevo.");
      }
    } catch (error) {
      console.log(error);
    }
          ...
          <div className="flex flex-row maxsm:flex-col-reverse items-start justify-start gap-x-5  maxmd:py-4 maxmd:px-5 maxsm:px-0">
            ...
            <div className="content-class w-1/2 maxsm:w-full h-full ">
              <div className="flex flex-col items-start justify-start pt-10 maxsm:pt-2 gap-y-10 w-[90%] maxmd:w-full p-5 pb-10">
                ...
                >
                  <p className="text-7xl font-semibold font-primary">
                    {post?.title}
                  </p>
