...
import { formatSimpleDate } from "@/backend/helpers";

const Profile = () => {
  const { user } = useContext(AuthContext);
  return (
    <>
      <figure className="flex maxsm:flex-col items-start sm:items-center text-black">
        <div className="relative flex ">
          {user?.image ? (
            ...
            <div className="w-16 h-16 rounded-full mr-4 bg-black text-white flex items-center justify-center uppercase text-2xl font-raleway">
        ...
        <figcaption>
          ...
          <p className="flex items-center maxsm:text-sm">
            ...
            <span>
              {user?.createdAt && ` ${formatSimpleDate(user?.createdAt)}`}
            </span>
          </p>
        </figcaption>
      </figure>
